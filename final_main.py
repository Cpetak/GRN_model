...
class Agent:
    def __init__(self,ID,args):
        ...
        self.alpha = self.args.alpha #for sigmoid function - could be mutated later, now constant
        self.fitness = 0
    ...
    def to_dict(self):
        return {"fit":self.fitness, "N":self.N, "adj":self.adjM.tolist()}
...
def evolve(args):
    ...
    for i in range(args.gens):
        ...
        wandb.log({'expression_robustness': ave_exp_rob}, commit=False)
        wandb.log({'fitness_robustness': ave_fit_rob}, commit=False)
        wandb.log({'benefitial_mutation': ave_ben_mut}, commit=False)
        wandb.log({'deleterious_mutation': ave_del_mut}, commit=False)
        ...
        wandb.log({'low': lowt}, commit=False)
        wandb.log({'high': hight}, commit=False)
        wandb.log({'spec': spect}, commit=False)
        wandb.log({'gen': gent}, commit=False)
        ...
        wandb.log({'average_fitness': np.mean(pop_fitness)}, commit=True)

        pop_prob = [f/tot_fitness for f in pop_fitness]

        ch_pop =np.random.choice(pop, args.pop_size, p=pop_prob, replace = True) #same individual can be selected over and over again
        pop = [deepcopy(agent) for agent in ch_pop]
        ...
        with open(args.name_of_file + "_features.txt", "w") as f:
                f.write(json.dumps(best_agent.to_dict())+"\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-alpha', type=float, default=10, help="Alpha value for sigmoid function")
    ...
    parser.add_argument('-proj', type=str, default="GRN_evolution", help="Name of the project (for wandb)")

    args = parser.parse_args()

    tag = conf2tag(vars(args))
    wandb.init(config=args, project=args.proj, group=args.exp_type+tag)
